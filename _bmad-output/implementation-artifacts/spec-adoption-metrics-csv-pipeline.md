...
- 2026-07-23: Resolved two of the four deferred items from round 1. `extract_claude.py`'s `message_count` now excludes synthetic tool-result-echo `user` turns, matching Codex's rollout format (which never represents tool output as a message) — closes the cross-tool comparability gap. `pipeline.py` gained a row-count sanity check (50% drop threshold, judgment call, warns rather than blocks) on full unfiltered runs only. `project_path` redaction remains open (still no export/sharing feature to motivate it) — see `deferred-work.md`.
- 2026-07-23: Closed the final deferred item. Added `pipeline.py --redact-paths`: replaces `project_path` in `sessions.csv` with a stable one-way hash (`csv_common.redact_path`, sha256-based) so grouping by project still works on redacted output but the real path can't be recovered. `--repo` filtering still runs against the real path first (inside the extractors), so the two flags compose correctly. `inventory.py` accepts the flag as a documented no-op since `run.sh` forwards all flags to both scripts and `inventory.py` never prints paths. `deferred-work.md` is now empty.
