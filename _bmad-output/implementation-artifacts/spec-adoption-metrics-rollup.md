...
- 2026-07-23: Closed the two remaining deferred items from round 1. Added `is_partial` column (`1` if `period_start + days_in_period` extends past today, computed with an injectable `today` param for deterministic tests) instead of silently leaving the in-progress period's deflated `active_day_rate` unmarked. `main()` now warns to stderr (doesn't error) when every `data/<tool>/daily_activity.csv` parses to zero rows, since that could mean either "no data yet" or an upstream break.
