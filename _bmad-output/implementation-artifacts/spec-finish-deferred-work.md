...
- 2026-07-23: Both remaining items resolved on closer inspection/implementation. Nested subagent-of-subagent linkage turned out to already be correct — `fpath.parent.parent.name` resolves to the *immediate* parent at any nesting depth, not necessarily the top-level human; only the comment was misleading (fixed), confirmed with a new test using two levels of `subagents/` nesting. The cross-tool `message_count` comparability question is resolved in `extract_claude.py`: synthetic tool-result-echo `user` turns (content is only a `tool_result` block) are now excluded, matching Codex's rollout format where tool output is never represented as a message.
